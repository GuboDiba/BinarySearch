...
function merge(leftIndex,rightIndex){
    ...
    while(s<leftIndex.length && t < rightIndex.length){
        if(leftIndex[s]<rightIndex[t]){
            result.push(leftIndex[s]);
            s++;
            
            
        }else{
            result.push(rightIndex[t]);
            t++;
        }
        
    }
    return result.concat(leftIndex.slice(s)).concat(rightIndex.slice(t));
}
...
function mergeSortdescend(array2){
    if (array2.length <= 1){
        return array2;
    }
...
function mergee(leftIndex,rightIndex){
    const result = [];
    let a =0;
    let b=0;
